---
name: eas-hosting
description: "EAS service (płatny). Deploy stron Expo i API routes Expo Routera na EAS Hosting — export web bundle, `eas deploy` (produkcja + PR preview URLs), sekrety środowiskowe, custom domains, runtime Cloudflare Workers. Pokrywa też pisanie API routes (`+api.ts`, metody HTTP, obsługa requestów, CORS) — ukrywanie sekretów (OpenAI/Stripe/DB keys), proxy zewnętrznych API, webhooks, rate limiting, walidacja server-side. Używaj przy 'ukryć API key', 'proxy do OpenAI', 'webhook endpoint', '+api.ts', 'EAS Hosting', 'deploy backendu mobile'. Buildy natywne i sklepy → skill eas-app-stores."
version: 1.0.0
license: MIT
---
