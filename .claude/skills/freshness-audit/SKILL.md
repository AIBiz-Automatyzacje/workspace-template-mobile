...
description: "Cykliczny audyt aktualności skilli technicznych względem żywej dokumentacji (oficjalne docs, changelogi GitHub, npm registry). Weryfikuje wersje, pinowane paczki i wzorce API z żywych źródeł, nie z pamięci modelu. Używaj przy: „audyt aktualności", „freshness", „czy skille są aktualne", „sprawdź wersje w skillach", „czy Stripe/Supabase/Sentry/Maestro w skillach jest aktualny", przeglądzie przeterminowań w guidelines technicznych."
...
Sprawdza, czy skille techniczne (`ux-ui-guidelines-mobile`, `supabase-dev-guidelines`, `security`, `sentry-integration`, `mobile-e2e-maestro` + ich `resources/`) nie przeterminowały się względem **żywej** dokumentacji. Powstał po tym, jak audyt z pamięci modelu (2026-07-06, repo web) przegapił Stripe v22 — dlatego tu **każde ustalenie musi pochodzić z URL żywego źródła**, nie z pamięci modelu.

**Skille `expo-*` i `eas-*` są POZA zakresem audytu** — to skille upstream-owned (oficjalne repo `github.com/expo/skills`); ich świeżość utrzymujemy przez sync diff z upstreamem, nie przez własny audyt treści.
...
2. **Ustal zakres.** Jeśli użytkownik podał argument (`$1` = lista nazw skilli) — przekaż go jako `skille`, by zawęzić audyt. Bez argumentu workflow audytuje domyślną piątkę skilli technicznych (bez `expo-*`/`eas-*` — patrz wyżej).
