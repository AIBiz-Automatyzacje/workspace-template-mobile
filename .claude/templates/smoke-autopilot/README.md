...
## Jak uzyc (w projekcie docelowym, np. gramywpadla)
...
2. W `.autopilot-state.json` ustaw fazie 1: `"fix": "pending"` i wstaw 1 sztuczny finding do
   `otwarteFindingi` (np. `{"severity":"P2","typ":"TEST","plik":"lib/smoke-autopilot.ts","opis":"brakuje testu wartosci ujemnych"}`),
...
- Usun `lib/smoke-autopilot.ts` + test (jesli revert ich nie zdjal).
